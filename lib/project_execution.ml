...
module Fast = struct
  ...
  let generated_list_builder_id builder_id suffix =
    "__list_builder_" ^ builder_id ^ "_" ^ suffix

  let generated_list_builder_wire_id builder_id suffix =
    "__list_builder_" ^ builder_id ^ "_wire_" ^ suffix

  let generated_anchor port x y = { P.port = port; at = { P.x; y } }

  let generated_bounds x y width height = { P.x; y; width; height }

  let endpoint_for_list_builder_output builder_id item_ids =
    match item_ids with
    | [] ->
        P.Element_port
          { element_id = generated_list_builder_id builder_id "nil"; port = "value" }
    | first :: _ ->
        P.Element_port
          {
            element_id = generated_list_builder_id builder_id ("cons_" ^ first);
            port = "value";
          }

  let expand_list_builder_element (element : P.element) =
    match element.kind with
    | P.ListBuilder { item_type; item_ids } ->
        let base_x = element.bounds.x in
        let base_y = element.bounds.y in
        let nil =
          {
            P.id = generated_list_builder_id element.id "nil";
            kind = P.Nil item_type;
            bounds =
              generated_bounds (base_x + 96 + (List.length item_ids * 64))
                (base_y + 16 + (List.length item_ids * 28)) 96 56;
            port_anchors =
              [
                generated_anchor "value"
                  (base_x + 192 + (List.length item_ids * 64))
                  (base_y + 44 + (List.length item_ids * 28));
              ];
          }
        in
        let cons_nodes =
          item_ids
          |> List.mapi (fun index item_id ->
                 let x = base_x + 96 + (index * 64) in
                 let y = base_y + 16 + (index * 28) in
                 {
                   P.id =
                     generated_list_builder_id element.id ("cons_" ^ item_id);
                   kind = P.Cons item_type;
                   bounds = generated_bounds x y 128 72;
                   port_anchors =
                     [
                       generated_anchor "head" x (y + 20);
                       generated_anchor "tail" x (y + 52);
                       generated_anchor "value" (x + 128) (y + 36);
                     ];
                 })
        in
        nil :: cons_nodes
  ...
  let expand_list_builder_wire builders (wire : P.wire) =
    let remap_source = function
      | Some (P.Element_port { element_id; port = "result" }) -> (
          match List.assoc_opt element_id builders with
          | Some item_ids -> Some (endpoint_for_list_builder_output element_id item_ids)
          | None -> Some (P.Element_port { element_id; port = "result" }))
      | other -> other
    in
    let remap_target = function
      | Some (P.Element_port { element_id; port }) -> (
          match List.assoc_opt element_id builders with
          | Some item_ids when List.mem port item_ids ->
              Some
                (P.Element_port
                   {
                     element_id =
                       generated_list_builder_id element_id ("cons_" ^ port);
                     port = "head";
                   })
          | _ -> Some (P.Element_port { element_id; port }))
      | other -> other
    in
    {
      wire with
      source_hint = remap_source wire.source_hint;
      target_hint = remap_target wire.target_hint;
    }

  let list_builder_tail_wires builder_id item_ids =
    item_ids
    |> List.mapi (fun index item_id ->
           let next_source =
             ...
             | Some next_item_id ->
                 P.Element_port
                   {
                     element_id =
                       generated_list_builder_id builder_id
                         ("cons_" ^ next_item_id);
                     port = "value";
                   }
             | None ->
                 P.Element_port
                   {
                     element_id = generated_list_builder_id builder_id "nil";
                     port = "value";
                   }
           in
           {
             P.id = generated_list_builder_wire_id builder_id ("tail_" ^ item_id);
             points = [];
             source_hint = Some next_source;
             target_hint =
               Some
                 (P.Element_port
                    {
                      element_id =
                        generated_list_builder_id builder_id ("cons_" ^ item_id);
                      port = "tail";
                    });
           })

  let expand_list_builders_for_fast document =
    let builders =
      document.P.elements
      |> List.filter_map (fun (element : P.element) ->
             match element.kind with
             | P.ListBuilder { item_ids; _ } -> Some (element.id, item_ids)
             | _ -> None)
    in
    if builders = [] then document
    else
      let elements =
        document.P.elements |> List.concat_map expand_list_builder_element
      in
      let user_wires =
        document.P.wires |> List.map (expand_list_builder_wire builders)
      in
      let generated_wires =
        builders
        |> List.concat_map (fun (builder_id, item_ids) ->
               list_builder_tail_wires builder_id item_ids)
      in
      { document with elements; wires = user_wires @ generated_wires }
  ...
  and eval_element_port state document env element_id port =
    ...
    | Some element -> (
        ...
        | P.ListBuilder _, _ ->
            fail
              "Fast execution expected List Builder to be lowered to Nil/Cons before evaluation."
        | P.Unpair _, "left" -> (
  ...
  let execute document =
    let document = expand_list_builders_for_fast document in
    match
      List.find_opt
