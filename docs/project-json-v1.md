...
`view` is optional and non-semantic. All other shown fields are required.
Unknown fields are rejected at every object level. This strict v1 policy
prevents misspelled semantic data from being silently discarded. A later
compatible extension therefore requires a new supported version.
...
The encoder emits a trailing newline and a fixed object-key order. It sorts
...
Encoding the same typed document is byte-for-byte deterministic. Decode after
encode preserves document meaning. Geometry validation and inference already
canonicalize non-semantic input list order, so reordering those arrays cannot
change compilation.
...
Saved data is declarative project geometry plus optional camera view. v1 does
not save selection, hover, drag state, menus, panels, undo history, execution
results, runtime values, rewrite traces, inferred symbolic relations, lowered
Core graphs, caches, layout, routing, or SVG DOM.
