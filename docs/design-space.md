...
| Initial implementation scope | Engine implementation | Provisional | `Unit + Nat + Succ + Copy + Drop + Parameter/Result boundaries` | literals only; Succ/Drop only; add Function/Apply; add NatRec | Determines which fixtures can validate and execute now | Avoids premature recursion and function validation choices | Conformance for this subset starts with validation acceptance/rejection and minimal runtime behavior | Function, Apply, NatRec, dynamic PrioritySpine, and multi-scope scheduling remain absent |
...
| CallFrame representation | Semantics | Provisional | `internal-runtime-return-link` | expose as Surface object; implicit host stack; encode only in trace | Affects replay and return provenance | No direct effect | Conformance must preserve caller/apply/callee/return target relation | Internal runtime concept, not Surface syntax |
...
| Copy semantics | Semantics | Provisional | `consume-one-create-two-distinct-linear-values` | observe without consume; alias input; implicit fan-out | Changes consumed/created trace entries and downstream logical identity | Explicit finite duplication adds no recursion | Conformance compares one consumed value and two created values | Implemented for `Unit` and `Nat`; closure copying remains open |
...
| Implemented rewrite subset | Semantics | Provisional | `Succ + Copy + Drop + Function` with static `PrioritySpine` scheduling | validation only; Succ/Drop only; add Apply; add NatRec | Determines executable traces and results | `Succ`, `Copy`, `Drop`, and closure creation do not add recursion | Conformance includes exact event order, Copy created-value order, Function capture order, PrioritySpine same-epoch selection, and result for this subset | No Apply, NatRec, dynamic spine, or multi-scope runtime |
| Function closure creation | Semantics | Provisional | `implemented-template-reference-and-ordered-captures` | defer until Apply; host closure payload; inline template body copy | Changes validation, ready state, trace rule, and Arrow value representation | Closure creation itself adds no recursion | Conformance includes template lookup, capture order, consumed IDs, and one created closure | Function body execution remains deferred to Apply |
