...
| Runtime instance identity | Semantics | Provisional | per-`Apply` logical instance with deterministic caller/apply/event-derived ID | reuse template graph directly; inline expansion without instance identity; host stack frame only | Changes snapshots, trace subjects, and debugging behavior | Separate instances avoid hidden shared execution state | Conformance targets logical instance identity | Current string form is provisional and not final serialization |
