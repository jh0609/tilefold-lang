...
| Initial implementation scope | Engine implementation | Provisional | `Unit + Nat + Succ + Drop + Parameter/Result boundaries` | literals only; add Copy; add Function/Apply; add NatRec | Determines which fixtures can validate now | Avoids premature recursion and function validation choices | Conformance for this subset starts with validation acceptance/rejection | Rewrites are still absent |
