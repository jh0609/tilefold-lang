...
| Spatial ordering relation | Semantics | Open | no implicit pixel order | `Before`; `Priority`; `Sequence`; no ordering relation | Could affect rewrite order only if explicitly semantic | Must preserve deterministic total execution | Must be recorded in canonical program and profile if introduced | Not a Core primitive in `transparent-v0` |
