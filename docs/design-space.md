...
| Implemented rewrite subset | Semantics | Provisional | `Succ + Copy + Drop` with static `PrioritySpine` scheduling | validation only; Succ/Drop only; add Apply; add NatRec | Determines executable traces and results | `Succ`, `Copy`, and `Drop` do not add recursion | Conformance includes exact event order, Copy created-value order, PrioritySpine same-epoch selection, and result for this subset | No Apply, Function, NatRec, dynamic spine, or multi-scope runtime |
