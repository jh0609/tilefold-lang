...
| Initial implementation scope | Engine implementation | Provisional | `Unit + Nat + Succ + Copy + Drop + Function + Apply + Parameter/Capture/Result boundaries` | literals only; Succ/Drop only; add Function/Apply; add NatRec | Determines which fixtures can validate and execute now | Avoids premature `NatRec` choices | Conformance for this subset starts with validation acceptance/rejection and minimal runtime behavior | NatRec, dynamic PrioritySpine, and multi-scope scheduling remain absent |
...
| Implemented rewrite subset | Semantics | Provisional | `Succ + Copy + Drop + Function + ApplyEnter + ApplyReturn` with static per-instance `PrioritySpine` scheduling | validation only; Succ/Drop only; add Function; add NatRec | Determines executable traces and results | Current Apply rejects template dependency cycles and adds no general recursion | Conformance includes exact event order, instance IDs, scoped value IDs, Copy created-value order, Function capture order, Apply enter/return order, PrioritySpine same-epoch selection, and result for this subset | No NatRec, dynamic spine, or interleaved multi-scope runtime |
