...
| Implemented rewrite subset | Semantics | Provisional | `Succ + Copy + Drop` | validation only; Succ/Drop only; add Apply; add NatRec | Determines executable traces and results | `Succ`, `Copy`, and `Drop` do not add recursion | Conformance includes exact event order, Copy created-value order, and result for this subset | No Apply, Function, NatRec, or PrioritySpine runtime |
